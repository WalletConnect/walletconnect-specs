...
WalletConnect exposes a `HTTP GET /auth-nonce?did=<client_id>` endpoint to retrieve a nonce. The value of the nonce is `websocketId` generated by the relay server, with a length of 32 bytes.
...
The Bearer is a standard signed JWT token which contains the authentication nonce as subject and did as the issuer. It has a a jwt [expiration](https://datatracker.ietf.org/doc/html/rfc7519#section-4.1.4) of **30 seconds** using `NumericDate` [standard (last definition in the section)](https://www.rfc-editor.org/rfc/rfc7519#section-2).
