...
WalletConnect expects an `Authentication Bearer <signed_jwt>` header when establishing the Websocket connection. Where websocket headers are not supported i.e browsers, use url query param `?auth=<signed_jwt>`. Server should support both of these mechanisms.
