...
WalletConnect expects an `Authentication Bearer <signed jwt>` header when establishing the Websocket connection. Where websocket headers are not supported i.e browsers url query param can be used `?auth=<signed jwt>`. Server should support both of these mechanisms.
